...
export const AuthModal: FC<AuthModalProps> = ({
  ...
  const handleLoginSubmit = (e: FormEvent) => {
    ...
    if (!query) {
      setLoginError('يرجى إدخال رقم هاتفك أو اسمك المسجل.');
      return;
    }

    const cleanQ = query.toLowerCase().replace(/[\s\-_]/g, '');
    
    // Private lookup: search volunteers by phone, email, or exact/partial name
    const target = volunteers.find((v) => {
      const vPhone = (v.phone || '').replace(/[\s\-_]/g, '');
      const vEmail = (v.email || '').toLowerCase();
      const vName = v.name.toLowerCase().replace(/[\s\-_]/g, '');
      return vPhone === cleanQ || vEmail === query.toLowerCase() || vName === cleanQ || vName.includes(cleanQ);
    });

    if (target) {
      if (target.status === 'معطل') {
        setLoginError('تنبيه: هذا الحساب معطل مؤقتاً بقرار إداري من دار الشباب الروينة. يرجى مراجعة إدارة المركز.');
        return;
      }
      onLogin(target);
      onClose();
    } else {
      setLoginError(`لم نعثر على متطوع مسجل بالرقم أو الاسم "${query}".`);
    }
  };
          ...
          <form onSubmit={handleLoginSubmit} className="p-5 space-y-4 text-right">
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1.5">
                أدخل رقم هاتفك أو اسمك المسجل للدخول:
              </label>
              ...
              <div className="relative mb-2">
                ...
                <input
                  type="text"
                  placeholder="مثال: 0550123456 أو اسم المتطوع..."
                  value={loginInput}
                  onChange={(e) => {
              ...
              {loginError && (
                <div className="mb-2.5 p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs flex flex-col gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setName(loginInput.trim());
                      setPhone(loginInput.trim().match(/^[0-9+]+$/) ? loginInput.trim() : '');
                      setActiveTab('register');
                      setLoginError(null);
                    }}
              ...
              <p className="text-[11px] text-slate-400 leading-relaxed">
                خصوصيتك محمية: لا يتم إظهار أرقام هواتف أو أسماء المتطوعين إلا لإدارة دار الشباب في لوحة التحكم الإدارية.
              </p>
            </div>
